...
def main():
    parser = argparse.ArgumentParser(description="Handelsregister Scheduler - Continuous monitoring")
    parser.add_argument("--db", default="handelsregister.db", help="Database file path")
    parser.add_argument(
        "--discovery-interval", type=int, default=2, help="Hours between discovery job runs (default: 2)"
    )
    parser.add_argument("--run-now", action="store_true", help="Run jobs immediately when starting")
    parser.add_argument("--discovery-only", action="store_true", help="Run single discovery job and exit")
    parser.add_argument("--backfill-only", action="store_true", help="Run single backfill job and exit")
    parser.add_argument("--regscan-only", action="store_true", help="Run single registration scan job and exit")
    parser.add_argument("--status", action="store_true", help="Show scheduler status and exit")
    parser.add_argument("--rate-status", action="store_true", help="Show rate limiter status and exit")
    parser.add_argument("--max-requests", type=int, default=20, help="Maximum requests per job run (default: 20)")
    ...
    if args.status:
        ...
        try:
            ...
            completed = conn.execute("SELECT COUNT(*) FROM backfill_state WHERE status = 'completed'").fetchone()[0]
            print("\nBackfill Progress:")
            print(
                f"  {completed}/{total} combinations ({completed / total * 100:.1f}%)" if total > 0 else "  Not started"
            )
            conn.close()
        except:
        ...
        try:
            ...
            if scan_states:
                ...
                for s in scan_states:
                    print(
                        f"  {s['court_code']} ({s['registry_type']}): "
