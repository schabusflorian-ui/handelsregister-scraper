...
class StartupDatabase:
    ...
    def upsert_company(self, company_data: Dict) -> bool:
        ...
        cursor.execute('SELECT entity_id FROM companies WHERE entity_id = ?', 
    ...
    def log_scrape(self, keywords: List[str], companies_found: int, 
                   new_companies: int, new_raises: int):
...
def run_monitoring_scrape():
    ...
    for company in results:
        ...
        # Add capital raises
        raises_count = db.add_capital_raises(
            company['entity_id'], 
            company['capital_raises']
        )
