...
class Database:
    ...
    def _create_tables(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_enrichment_queue_priority ON enrichment_queue(priority, queued_at)"
        )
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_investor_entities_investor ON investor_legal_entities(investor_id)"
        )
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_news_articles_early_stage ON news_articles(is_early_stage_related)"
        )
        ...
        self._migrate_stealth_founders_table(cursor)
    ...
    def _migrate_news_tables(self, cursor):
        ...
        try:
            ...
            if "is_early_stage_related" not in columns and columns:
                cursor.execute("ALTER TABLE news_articles ADD COLUMN is_early_stage_related INTEGER DEFAULT 0")
                self.conn.commit()
        except Exception:
        ...
        try:
            ...
            if "early_stage_signals" not in columns and columns:
                cursor.execute("ALTER TABLE news_alerts ADD COLUMN early_stage_signals TEXT")
                self.conn.commit()
        except Exception:
            pass
    ...
    def upsert_company(self, company_number: str, name: str, source: str, **kwargs) -> Tuple[int, bool]:
        """
    ...
    def add_to_enrichment_queue(self, company_id: int, priority: int = 5, reason: str = "new_company"):
    ...
    def get_news_alerts(
        ...
        if alert_type:
            cursor.execute(
                "SELECT * FROM news_alerts WHERE alert_type = ? ORDER BY created_at DESC LIMIT ?", (alert_type, limit)
            )
        else:
            cursor.execute("SELECT * FROM news_alerts ORDER BY created_at DESC LIMIT ?", (limit,))
        return [dict(row) for row in cursor.fetchall()]
    ...
    def get_early_stage_articles(self, limit: int = 100) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT * FROM news_articles WHERE is_early_stage_related = 1 ORDER BY fetched_at DESC LIMIT ?", (limit,)
        )
        return [dict(row) for row in cursor.fetchall()]
    ...
    def _classify_founder_change(self, field: str, old_value: Optional[str], new_value: Optional[str]) -> str:
        ...
        if field == "headline":
            ...
            became_founder = any(w in new_lower for w in founder_words) and not any(
                w in old_lower for w in founder_words
            )
    ...
    def set_scan_watermark(
        ...
        if row:
            ...
            """,
                (last_scanned_number, now, total_scanned, total_found, court_code, registry_type),
            )
        else:
            self.conn.execute(
                """
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (court_code, registry_type, last_scanned_number, now, scanned_count, found_count),
            )
