...
class HandelsregisterAIScraper:
    ...
    def search_by_keyword(self, keyword: str, max_results: int = 50) -> List[Dict]:
        ...
        try:
            ...
            results = self.client.search_organizations(q=keyword, limit=max_results)
    ...
    def analyze_company(self, company_data: Dict) -> Dict:
        ...
        analysis["ai_robotics_score"] = sum(
            1 for keyword in self.AI_ROBOTICS_KEYWORDS if keyword.lower() in purpose_text
        )
        ...
        for keyword in keywords:
            ...
            for company_summary in companies:
                ...
                if recent_months and not analysis["is_recent"]:
                    print("  Skipped: Not a recent incorporation")
                    continue

                print(
                    f"  ✓ Match! Score: {analysis['ai_robotics_score']}, Capital raises: {len(analysis['capital_raises'])}"
                )
...
def main():
    ...
    if results:
        ...
        print("\n✓ Scraping complete!")
        print("  - CSV: ai_robotics_startups.csv")
        print("  - JSON: ai_robotics_startups.json")
