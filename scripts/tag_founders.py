...
GEO_MAP = {
    ...
    "nrw": ["düsseldorf", "köln", "cologne", "dortmund", "essen", "bonn", "nordrhein", "north rhine", "nrw"],
...
def classify_ex_companies(headline: str, signals: list) -> tuple:
    ...
    if headline:
        for match in re.finditer(
            r"(?:ex[- ])(\w[\w& .]*?)(?:\s*[|,\-–]|\s+(?:Senior|Director|VP|Head|Manager|Engineer|Architect|Founder|CEO|CTO|at )|\s*$)",
            headline,
            re.IGNORECASE,
        ):
...
def run_tagger(db_path: str, retag: bool = False):
    ...
    if retag:
        founders = db.execute("SELECT * FROM stealth_founders").fetchall()
    else:
        founders = db.execute("SELECT * FROM stealth_founders WHERE data_quality IS NULL").fetchall()
